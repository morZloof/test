...
function runningTimesJs(){
	...
	function showBuildingsOnBuildingProocess(jsonBuildings,jsonReq) {
     ...
     function createBuildingsTime(json){
        if (json.length > 0) {
            ...
            for (var i=0; i<loopLength; i++ ) {
                ...
                $(oneBuilding).find('.runningTimes_finishWithRep').find('span').text(choicePopup.getReputation(json[i].dateEnd))
    ...
    function createFactoryReqTime(json){
        if (json.length > 0) {
            ...
            for (var i=0; i<loopLength; i++ ) {
                ...
                var oneBuilding = $('.runningTimes_oneBuilding:first').clone();
                counterDown(oneBuilding,json[i].endDate);
                ...
                $(oneBuilding).find('.runningTimes_finishWithRep').hide()
