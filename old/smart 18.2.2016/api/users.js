	...
	function getUser(res, req, id) {	
		async.waterfall([
			function(callback){
                ...
                + 'from users '
                + 'where profit>(select profit from users where id=' + id + '); '

				pool.query(sql,function(err,result){
                    var rank = result[2][0].newRank + 1;
					var maxMoney = result[1][0].maxMoney;
					var money = result[1][0].money;

					callback(null,result[1],maxMoney,money,rank);
				})
			},
			function (json,maxMoney,money,rank,callback){
				...
				pool.query(sql,function(err,result){
                    ...
                    + "SELECT * FROM factoryReq WHERE send_users_id=" + id + "; "
