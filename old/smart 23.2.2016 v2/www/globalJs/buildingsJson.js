...
function buildingsJsonJs(){
	...
	function getMainBuilding() {
        if($.globalUserBuilding==undefined || $.globalUserBuilding.length==0){
            return 0;
        }
    ...
    function counterBuildings(type_id){
        if($.globalUserBuilding== undefined){
            return 0;
        }
		...
		for (var i=0; i< loopLength; i++) {
			if ($.globalUserBuilding[i].type_id == '3' || $.globalUserBuilding[i].type_id==3) {
				counter ++;
			}			
		}
    ...
    this.counterBuildings = counterBuildings;
}