...
function warJs()
{
    ...
    function randomRestuarant(res,req,id){
        ...
        if(isRestuarant==1){
            var sql = 'SELECT * FROM usersBuildings WHERE type_id=2 AND other is null ORDER BY RAND() LIMIT 1;'
        }
        else{
            var sql = 'SELECT * FROM usersBuildings WHERE type_id=6 AND other is null ORDER BY RAND() LIMIT 1;'
        }
			...
			function(callback){
				...
				pool.query(sql,function(err,result){
					...
					if(type_id!=2 && type_id!=6){
						...
						callback(null,get_users_id,userName,other,factory_type_id,factoryBonus);
					}
				})
			},function(get_users_id,userName,other,factory_type_id,factoryBonus,callback){
