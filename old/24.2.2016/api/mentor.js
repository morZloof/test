	...
	function updateMentor(res,req,id){
		...
		pool.query(sql,function(err,result){
			...
			if(mentorLevel == 1 && citys.length>0){
				...
				var sql = 'UPDATE users SET mentor=9 WHERE id= ' + id;
				userDetials.mentor=9;
				...
				var sql = 'UPDATE users SET '
                            + 'mentor=12 '
                        + 'WHERE id= ' + id;
				userDetials.mentor=12;
				
				pool.query(sql);
			}
            else if(mentorLevel == 12){
                var nowTime = new Date()
                nowTime = nowTime.addHours(2);
