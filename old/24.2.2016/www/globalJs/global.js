...
function globalJs(){
	...
	function loadJson(){
        ...
        $.json_userLevels = json_userLevels;
	}
	...
	function getUser(){
		...
		global.api(url, data, function(data) {
            ...
            $.globalUserFactoryReq = data[6];
            $.globalUserBonus = data[7];
	...
	function setUser(){
		...
		runningTimes.showBuildingsOnBuildingProocess($.globalUserOnBuildingProcess,$.globalUserFactoryReq,$.globalUserBonus);
        ...
        notebookNav.showPage();
        
        if(location.href.toString().indexOf('besmarter')>0){
            hj('tagRecording', [$.globalUser.userName]);
        }
